      ...
      <div>
        ...
        <div class="condition-panel-range-set">
          ...
          <ConditionRangeBetween
            ...
            :data.sync="data1"
          />
          <ConditionInputTextarea
            v-else-if="data1.key === 'values'"
            :value.sync="inputVal"
          />
          <ConditionInputNumber v-else :value.sync="inputVal" />
        </div>
      </div>
    </div>
...
import ConditionInputTextarea from './condition-input-textarea'
...
export default {
  ...
  data() {
    return {
      ...
      items: [
        ...
        { key: 'values', label: '固定值集合(用,分割)', value: '' } //扩展固定值的集合 20220428 liuhj
      ]
    }
  },
  computed: {
  ...
  watch: {
    ...
    inputVal(val) {
      ...
      this.data1.type = 'dingtalk_actioner_range_condition' //先默认下避免出问题，为了扩展dingtalk_actioner_value_condition 20220428 liuhj
      switch (key) {
        case 'lt':
        ...
        case 'between':
          this.data1.boundEqual = val
          break
        case 'values': // 扩展dingtalk_actioner_value_condition 20220428 liuhj
          this.data1.paramValues = val.split(',')
          this.data1.type = 'dingtalk_actioner_value_condition'
          break
        default:
      }
    }
      ...
      if (val.key && val.key !== '') {
        switch (val.key) {
          ...
          case 'between':
            ...
            break
          case 'values': // 扩展dingtalk_actioner_value_condition 20220428 liuhj
            this.inputVal = val.paramValues.join(',')
            val.label = '固定值集合(用,分割)'
            this.data1.type = 'dingtalk_actioner_value_condition'
            break
          default:
        }
